...
export const SEARCH_INTENT_JSON_SCHEMA = {
  name: "search_intent",
  schema: {
    type: "object",
    properties: {
...
/**
 * Groq Client for Intent Parsing with Structured Outputs (openai/gpt-oss-120b)
 */
export async function callGroqStructuredIntent(
  userQuery: string,
  targetModule: "jobs" | "exams" | "all" | "bulletins" = "all"
): Promise<{ intent: StructuredSearchIntent | null; error?: string; rawContent?: string }> {
...
Examples:
- "Bihar me 10th pass sarkari naukri" -> {"module": "jobs", "query": null, "state": "Bihar", "state_code": "BR", "qualification": ["10th"], "category": null, "employment_type": null, "status": "active", "application_open": true, "gender": null, "salary_min": null, "salary_max": null, "deadline_before": null, "sort": "relevance"}
- "UP Police SI salary above 50000" -> {"module": "jobs", "query": "Police SI", "state": "Uttar Pradesh", "state_code": "UP", "qualification": null, "category": "Police", "employment_type": null, "status": "active", "application_open": true, "gender": null, "salary_min": 50000, "salary_max": null, "deadline_before": null, "sort": "salary"}`;
  ...
  try {
    const response = await fetch(config.endpoint, {
      ...
      headers: {
        ...
        "User-Agent": "SuchnaSetu-AI-Groq/1.0",
      },
      body: JSON.stringify({
        model: config.model,
        messages: [
          {
            role: "system",
            content:
              "You are a recruitment search intent extraction engine. Respond with a valid JSON object ONLY. Never include markdown backticks or conversational prose.",
          },
          {
            role: "user",
            content: prompt,
    ...
    if (!response.ok) {
      ...
      return { intent: null, error: `GROQ_HTTP_${response.status}` };
    }
    ...
    if (firstBrace === -1) {
      return { intent: null, error: "NO_JSON_OBJECT", rawContent };
    }
    ...
    return { intent: parsed as StructuredSearchIntent, rawContent };
  } catch (err: any) {
    clearTimeout(timeoutId);
    const isTimeout = err.name === "AbortError";
    console.warn(`[Groq Intent Parsing Failed: ${isTimeout ? "Timeout" : err.message}]`);
 ...
 * @deprecated Use callGroqStructuredIntent
 */
export const callOpenRouterStructuredIntent = callGroqStructuredIntent;
