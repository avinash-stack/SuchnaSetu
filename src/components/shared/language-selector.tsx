...
interface LanguageSelectorProps {
  variant?: "masthead" | "navbar" | "mobile";
  className?: string;
}

export function LanguageSelector({ variant = "masthead", className = "" }: LanguageSelectorProps) {
  ...
  const buttonStyle =
    variant === "masthead"
      ? "bg-white/10 hover:bg-white/20 text-white border-white/20 text-[11px]"
      : variant === "navbar"
      ? "bg-white hover:bg-slate-50 text-slate-800 border-slate-300 text-xs"
      : "w-full justify-between bg-slate-50 text-slate-900 border-slate-200 text-sm";
  ...
  return (
    <div className={`relative inline-block text-left ${className}`} ref={dropdownRef}>
      <button
        ...
        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xs font-semibold border transition-colors shadow-2xs ${buttonStyle}`}
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 z-50 mt-1 w-52 rounded-xs border border-slate-200 bg-white p-1.5 shadow-lg animate-in fade-in-50 zoom-in-95">
