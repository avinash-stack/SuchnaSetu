...
async function runComparison() {
  console.log('================================================================');
  console.log('SUCHNASETU POST-OPTIMIZATION LIGHTHOUSE & PERFORMANCE AUDIT');
  console.log('================================================================\n');

  const baselineData = JSON.parse(fs.readFileSync('scripts/lighthouse-baseline.json', 'utf-8'));
  const afterResults = [];

  for (const page of TARGET_PAGES) {
    console.log(`Auditing (Optimized): ${page.name} (${page.url})...`);

    // 1. Measure TTFB & Server response
    const ttfbInfo = await measureTtfb(page.url);

    // 2. Run Lighthouse in headless mode and output JSON
    const reportPath = `/tmp/lighthouse-after-${Date.now()}.json`;
    try {
      execSync(
      ...
      afterResults.push(metrics);
      console.log(`  -> Score: ${perfScore}/100 | LCP: ${metrics.lcpMs}ms | FCP: ${metrics.fcpMs}ms | TBT: ${metrics.tbtMs}ms | CLS: ${metrics.cls} | TTFB: ${metrics.ttfbMs}ms | Requests: ${metrics.totalRequests}`);
    } catch (err) {
  ...
  const comparisonTable = afterResults.map((after, idx) => {
    const before = baselineData[idx] || {};
    return {
      Page: after.name,
      'Score (Before -> After)': `${before.score || 0}/100 -> ${after.score}/100`,
      'LCP (Before -> After)': `${before.lcpMs || 0}ms -> ${after.lcpMs}ms`,
      'FCP (Before -> After)': `${before.fcpMs || 0}ms -> ${after.fcpMs}ms`,
      'TBT (Before -> After)': `${before.tbtMs || 0}ms -> ${after.tbtMs}ms`,
      'CLS (Before -> After)': `${before.cls || 0} -> ${after.cls}`,
      'TTFB (Before -> After)': `${before.ttfbMs || 0}ms -> ${after.ttfbMs}ms`,
      'Requests (Before -> After)': `${before.totalRequests || 0} -> ${after.totalRequests}`,
    };
  });

  console.table(comparisonTable);
  fs.writeFileSync('scripts/lighthouse-after.json', JSON.stringify(afterResults, null, 2));
  console.log('\nComparison data saved to scripts/lighthouse-after.json');
}
