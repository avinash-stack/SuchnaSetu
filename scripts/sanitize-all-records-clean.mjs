...
const VERIFIED_ORG_GATEWAYS = {
  ...
  'SSB': { apply: 'http://ssbrectt.gov.in', notif: 'http://ssbrectt.gov.in' },
  ...
  'NTPC': { apply: 'https://ntpc.co.in', notif: 'https://ntpc.co.in' },
...
async function sanitizeAllRecordsCompletely() {
  console.log('--- Sanitizing ALL Jobs and Exams to Verified Gateways ---');
  ...
  for (const j of (jobs || [])) {
    ...
    // If apply URL is not matching the official gateway or is broken
    let newApply = gateway.apply;
    let newNotif = gateway.notif;

    if (j.official_apply_url !== newApply || j.official_notification_url !== newNotif) {
  ...
  for (const e of (exams || [])) {
    ...
    let newWeb = gateway.apply;
    let newNotif = gateway.notif;

    if (e.official_website_url !== newWeb || e.official_notification_url !== newNotif) {
  ...
  console.log(`Systematic Sanitization Complete! Jobs: ${jobsUpdated}, Exams: ${examsUpdated}`);
}

sanitizeAllRecordsCompletely().catch(console.error);
